...
import L2PSwapV2Factory from '@l2pswap/core/build/L2PSwapV2Factory.json'
...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  const pair = new Contract(pairAddress, JSON.stringify(IL2PSwapV2Pair.abi), provider).connect(wallet)
  ...
  const WETHPair = new Contract(WETHPairAddress, JSON.stringify(IL2PSwapV2Pair.abi), provider).connect(wallet)
